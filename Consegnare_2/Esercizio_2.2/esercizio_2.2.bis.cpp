...
*/	
	for(int i=0; i<M; i++){ //ciclo sui blocchi
		...
		for(int j=0; j<L; j++){ //entro nel blocco j
			...
			for(int l=0; l<passi; l++){ //faccio random walk di 100 passi
				...
				double r_2 = P.getR() * P.getR(); //in questo modo il passo 0 è sempre 0
				v.push_back(r_2);
	
				double theta = rnd.Rannyu(0., M_PI); //genero angolo rispetto a asse z
				double phi = rnd.Rannyu(0., 2*M_PI); //genero angolo rispetto a asse x
			
				P.setX(P.getX() + passo * sin(theta) * cos(phi));
				P.setY(P.getY() + passo * sin(theta) * sin(phi));
				P.setZ(P.getZ() + passo * cos(theta));
	...
	for (int i=0; i<passi; i++){
		...
		Print("media_random_walk_continuo.dat", sqrt(ave_r[i]/M));
		Print("errore_random_walk_continuo.dat", err[i]/(2*sqrt(ave_r[i]/M)));
