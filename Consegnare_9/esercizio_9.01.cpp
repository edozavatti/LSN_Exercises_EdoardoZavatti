...
int main (int argc, char *argv[]){
   ...
   int seed[4];
    ...
    Matrix = Generate_Dist_circ(theta, N);
    //Print(Matrix);
    ...
    stampa.open("Route_0", ios::app);
    for(int i = 0; i < N; i++){
        double x_i = cos(theta[Population[0][i]-1]);
        double y_i = sin(theta[Population[0][i]-1]);
        stampa << x_i << " " << y_i << " " << endl;
    }
    stampa.close();
   ...
   int n=1;
   while(n < 201){
       string filename = "Route_" + to_string(n);
        ...
        media.open("Media",ios::app);
        media_best.open("Media_migliore", ios::app);
        ...
        if(n%100 == 0){
            ...
            stampa << cos(theta[Population[0][0]-1]) << " " << sin(theta[Population[0][0]-1]) << endl;
            stampa.close();
        }
        n++;
