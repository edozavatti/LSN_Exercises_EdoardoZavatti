...
int main (int argc, char *argv[]){
   ...
   int seed[4];
	...
	Print(Population);
    stampa.open("Route_00", ios::app);
    for(int i = 0; i < N; i++){
        double x_i = cos(theta[Population[i]-1]);
        double y_i = sin(theta[Population[i]-1]);
        stampa << x_i << " " << y_i << " " << endl;        
        cout << "stampo " << i << endl;
    }
    stampa << cos(theta[0]) << " " << sin(theta[0]) << " " << endl;
    stampa.close();
   ...
   while(T >= 0.){
    ...
    stampa.open("Route_fin", ios::app);
    for(int i = 0; i < N; i++){
        double x_i = cos(theta[Population[i]-1]);
        double y_i = sin(theta[Population[i]-1]);
        stampa << x_i << " " << y_i << " " << endl;
    }
    stampa << cos(theta[Population[0]-1]) << " " << sin(theta[Population[0]-1]) << endl;
    stampa.close();
