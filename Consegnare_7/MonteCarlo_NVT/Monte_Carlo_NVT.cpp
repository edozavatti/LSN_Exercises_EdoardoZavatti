...
//measurement of g(r)
  igofr = 2;
  nbins = 500; //numero di intervalli in cui divido lo spazio in cui varia r
...
//Evaluate potential energy and virial of the initial configuration
  Measure(); //misura le proprietà di energia potenziale, viriale, e pressione
...
void Measure()
{
  ...
  /*ene.open("Esercizio_7.1/output_ene_gas.dat", ios::app);
  pres.open("Esercizio_7.1/output_pres_gas.dat", ios::app);
  const int wd=12;
  */
...
// Devo identificare il bin relativo alla distanza r a cui associare l'incremento e aumentare questo di 2 (perchè l'algoritmo cicla solo una volta)
// r deve andare fino a l/2, non posso misurare distanze più grandi di l/2*sqrt(3)
// inserire una condizione di controllo che sia minore di l/2
// tutto ciò che va da zero a binsize deve cadere nel primo bin, da zero a 2 binsize deve cadere nel secondo bin, ...
// al primo bin darò il valore binsize/2, al secondo binsize + binsize/2, al terzo 2*binsize + binsize/2, ...

    if(dr < box/2.){  
      bin = dr/bin_size; //trovo il bin da aumentare
      bin += 2;
...
// contribution to energy and virial
  ...
  ene << walker[iv]/npart + vtail<< endl;
  pres << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("output.epot_solidMC.0",ios::app);
    Pres.open("output.pres_solidMC.0",ios::app);
    Gofr.open("output.gofr_solidMC.0",ios::app);
    Gave.open("output.gave_solidMC.0",ios::app);
